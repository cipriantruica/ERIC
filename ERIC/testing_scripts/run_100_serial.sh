...
do
	...
	python testing_mongo_serial.py $FILE $DELIMITER $HEADER $DB $LANGUAGE >> $path"100_perforance_serial"
	END=$(getCurrentTimeInMili)
	DIFF=$(( $END - $START ))
	echo "*******************" >> $path"100_perforance_serial"
	echo $DIFF >> $path"100_script_times_populatedb_serial"
done;
